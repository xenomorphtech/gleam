...
//    pub fn is_bool(&self) {
//        match self {
//            Type::Named { module, name, .. } => module == "gleam" && name == "Bool",
//            _ => false,
//        }
//    }
//}
    ...
    match p {
        ...
        Pattern::Constructor {
            ...
            if type_.is_bool() {
                ...
                tag_tuple_pattern(name, field_map, args, vars, define_variables, env)
            }
        }
...
fn tag_tuple_pattern<'a>(
    ...
    if args.is_empty() {
        ...
        let doc_args = args
            .iter()
            .enumerate()
            .filter(|(_, arg)| !arg.is_discard())
            .map(|(n, arg)| {
                let name = match rev_fields.get(&n) {
                    Some(name) => (*name).to_string(),
                    None => format!("field_{}", n),
                };
                Document::String(name).append(": ").append(print(
                    &arg.value,
                    vars,
                    define_variables,
                    env,
        ...
        //skip the __module__ part, assuming type checking works
        "%{ __type__: "
            .to_doc()
            .append(name)
            .append(", ".to_doc())
