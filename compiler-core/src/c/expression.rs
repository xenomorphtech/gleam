...
impl<'module> Generator<'module> {
    ...
    pub fn expression<'a>(&mut self, expression: &'a TypedExpr) -> Output<'a> {
        ...
        Ok(if expression.c_handles_own_return() {
            document
        } else {
    ...
    pub fn binop_child_expression<'a>(&mut self, expression: &'a TypedExpr) -> Output<'a> {
        match expression {
            TypedExpr::BinOp { name, .. } if name.c_is_operator_to_wrap() => {
...
impl TypedExpr {
    fn c_handles_own_return(&self) -> bool {
        matches!(
            self,
            TypedExpr::Try { .. }
                | TypedExpr::Todo { .. }
...
impl BinOp {
    fn c_is_operator_to_wrap(&self) -> bool {
        match self {
            BinOp::And
            | BinOp::Or
            | BinOp::Eq
